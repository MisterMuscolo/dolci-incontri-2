...
export const useUserNotifications = (userId: string | undefined) => {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    // Query per recuperare solo le notifiche associate esplicitamente a questo userId
    const { data, error } = await supabase
      .from('admin_notifications') // La tabella è 'admin_notifications' ma contiene anche notifiche utente
      .select('*')
      .eq('user_id', userId) // Filtra solo per le notifiche destinate a questo utente
