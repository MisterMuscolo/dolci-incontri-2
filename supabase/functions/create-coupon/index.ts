...
serve(async (req) => {
  ...
  try {
    const { code, type, discount_type, discount_value, expires_at, applies_to_user_id, max_uses } = await req.json();

    if (!code || !type || !discount_type || typeof discount_value !== 'number') {
      throw new Error('Missing required fields: code, type, discount_type, discount_value');
    }
    ...
    if (callerProfileError || callerProfile?.role !== 'admin') {
      return new Response(JSON.stringify({ error: 'Forbidden: Only admins can create coupons' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 403,
      });
    }

    // Use the service role key for the actual insert to bypass RLS for admin action
    ...
    const { data: newCoupon, error: insertError } = await supabaseAdmin
      ...
      .insert({
        ...
        applies_to_user_id: applies_to_user_id || null,
